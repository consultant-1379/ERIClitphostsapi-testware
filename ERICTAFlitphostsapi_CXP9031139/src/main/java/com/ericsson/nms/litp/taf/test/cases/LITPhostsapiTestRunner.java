...
public class LITPhostsapiTestRunner extends TorTestCaseHelper {
    
    Logger logger = Logger.getLogger(LITPhostsapiTestRunner.class);
    ...
    @TestId(id = "CXP9031139-1", title = "Upgrade ERIClitphostsapi rpm on the MS")
    @Test(groups={"ACCEPTANCE"})
    public void upgradeERIClitphostsapiRPM() throws TimeoutException, FileNotFoundException {

        rpmUpgradeOperator.initialise("hostsapi");
     ...
     * @PRIORITY HIGH
     */
    @TestId(id = "CXP9031139-2", title = "Run python test cases for ERIClitphostsapi package")
    @Test(groups={"CDB_REGRESSION", "ACCEPTANCE"})
    public void runERIClitphostsapiTests() {
     ...
     * @PRE Execution of test {@link #runERIClitphostsapiTests()}
     * @PRIORITY HIGH
     * @param className
     * @param name
     * @param failures
     */
    @TestId(id = "CXP9031139-3", title = "Parse xml outputs from python tests")
